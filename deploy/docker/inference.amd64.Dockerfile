...
COPY deploy/docker/requirements.cpu-amd64.txt deploy/docker/requirements.cpu-amd64.txt
RUN python -m venv /opt/venv && uv pip sync --python /opt/venv/bin/python --require-hashes deploy/docker/requirements.cpu-amd64.txt
RUN find /opt/venv/lib/python3.12/site-packages/torch/bin -mindepth 1 -maxdepth 1 -type f ! -name torch_shm_manager -delete && \
    rm -r /opt/venv/lib/python3.12/site-packages/torch/include /opt/venv/lib/python3.12/site-packages/torch/share
